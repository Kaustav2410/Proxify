...
export const adminAuthentication = async (req:Request,res:Response,next:NextFunction):Promise<any>=>{

    try{
        const header = req.headers.authorization;
        const authToken = header?.split(" ")[1];
        console.log("Inside of admin middleware");
        ...
        if(payload.role!=="Admin"){
            console.log("Admin middleware blocked");
            return res.status(StatusCodes.FORBIDDEN).json({
                message:"Unauthorized access"
            })
        }
        console.log(payload,"Admin middleware role checking passed");
        // console.log(payload);
